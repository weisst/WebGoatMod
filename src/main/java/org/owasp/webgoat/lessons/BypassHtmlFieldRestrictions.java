...
public class BypassHtmlFieldRestrictions extends SequentialLessonAdapter {
	public final static A MANDIANT_LOGO = new A().setHref(
	...
	protected Element createContent(WebSession s) {
		...
		try {
			...
			ec.addElement(new org.apache.ecs.html.Select("select",
					allowedSelect));
			...
			Input disabledinput = new Input(Input.TEXT, "disabledinput",
					defaultdisabledinputtext);
			...
			if (submittedselect.equals("foo"))
				failed = true;
			if (submittedselect.equals("bar"))
				failed = true;
			...
			// check checkbox (note - if the box is not checked, this will throw
			// an exception, but that
			// is okay)
			if (s.getParser().getRawParameter("checkbox").equals("on"))
				failed = true;

			// check shortinput
			if (s.getParser().getRawParameter("shortinput").length() < 6)
				failed = true;

			// check disabledinput (note - if the field was not re-enabled, this
			// will throw an exception, but that
			// is okay)
			if (s.getParser().getRawParameter("disabledinput")
					.equals(defaultdisabledinputtext))
				failed = true;

			// check submitbutton
			if (s.getParser().getRawParameter("SUBMIT").equals(submittext))
				failed = true;
	...
	public String getInstructions(WebSession s) {
		...
		String instructions = "";
		if (chinese) {
			instructions = "下面的表格使用HTML表单字段限制，要通过这章节，需要在提交表单中提交不允许的值，"
					+"<b>必须在一次提交中提交全部字段和值，值需要不符合默认规定。</b>";
		} else {
			instructions = "The form below uses HTML form field restrictions. "
