 ...
 * 
 * @author Ryan Knell <a href="http://www.aspectsecurity.com">Aspect
 *         Security</a>
...
public class ConcurrencyCart extends LessonAdapter {
	...
	protected Element createContent(WebSession s) {
		...
		try {
			...
			if ("Purchase".equalsIgnoreCase(submit)) {
				updateQuantity(s);
				ec = createPurchaseContent(s, quantity1, quantity2, quantity3,
						quantity4);
				...
				if (calcTOTAL > runningTOTAL) {
					...
					s.setMessage("Thank you for shopping! You have (illegally!) received a "
							+ discount
					...
					s.setMessage("You are on the right track, but you actually overpaid by "
							+ discount + "%. Try again!");
				}
			} else {
				updateQuantity(s);
				ec = createShoppingPage(s, quantity1, quantity2, quantity3,
						quantity4);
			}
	...
	private ElementContainer createPurchaseContent(WebSession s, int quantity1,
			int quantity2, int quantity3, int quantity4) {
		...
		try {
			...
			// Table Setup
			TR tr = new TR();
			tr.addElement(new TH().addElement("Shopping Cart Items").setWidth(
					"80%"));
			...
			tr.addElement(new TD()
					.addElement("Hitachi - 750GB External Hard Drive"));
			tr.addElement(new TD().addElement("$169.00").setAlign("right"));
			tr.addElement(new TD().addElement(String.valueOf(quantity1))
					.setAlign("center"));
			...
			tr = new TR();
			tr.addElement(new TD()
					.addElement("Hewlett-Packard - All-in-One Laser Printer"));
			tr.addElement(new TD().addElement("$299.00").setAlign("right"));
			tr.addElement(new TD().addElement(String.valueOf(quantity2))
					.setAlign("center"));
			...
			tr = new TR();
			tr.addElement(new TD()
					.addElement("Sony - Vaio with Intel Centrino"));
			tr.addElement(new TD().addElement("$1799.00").setAlign("right"));
			tr.addElement(new TD().addElement(String.valueOf(quantity3))
					.setAlign("center"));
			...
			tr.addElement(new TD().addElement(String.valueOf(quantity4))
					.setAlign("center"));
			...
			table = new Table().setCellSpacing(0).setCellPadding(2)
			...
			tr.addElement(new TD().addElement("$" + formatFloat(runningTOTAL))
					.setAlign("right"));
			table.addElement(tr);
			...
			tr.addElement(new TD().addElement(
					new Input(Input.TEXT, "CC", param2)).setAlign("right"));
			table.addElement(tr);
			...
			tr.addElement(new TD().addElement(
					new Input(Input.TEXT, "PAC", param1)).setAlign("right"));
			table.addElement(tr);

			// Confirm Button
			Element b = ECSFactory.makeButton("Confirm");
			tr = new TR();
			tr.addElement(new TD().addElement(b).setColSpan(2)
					.setAlign("right"));
			table.addElement(tr);
			...
			tr = new TR();
			tr.addElement(new TD().addElement(c).setColSpan(2)
					.setAlign("right"));
			table.addElement(tr);
	...
	private ElementContainer confirmation(WebSession s, int quantity1,
			int quantity2, int quantity3, int quantity4) {
		ElementContainer ec = new ElementContainer();
		...
		try {
			...
			ec.addElement(new HR().setWidth("90%"));
			ec.addElement(new Center().addElement(new H1()
					.addElement("Thank you for your purchase!")));
			ec.addElement(new Center().addElement(new H1()
					.addElement("Confirmation number: " + confNumber)));
			Table table = new Table().setCellSpacing(0).setCellPadding(2)
					.setBorder(1).setWidth("90%").setAlign("center");
			...
			tr = new TR();
			tr.addElement(new TD()
					.addElement("Total Amount Charged to Your Credit Card:"));
			tr.addElement(new TD().addElement("$" + formatFloat(runningTOTAL))
					.setAlign("right"));
			table.addElement(tr);
	...
	private ElementContainer createShoppingPage(WebSession s, int quantity1,
			int quantity2, int quantity3, int quantity4) {
		...
		try {
			...
			tr = new TR();
			tr.addElement(new TD().addElement(
					"Total: " + "$" + formatInt(subTOTAL) + ".00").setAlign(
					"left"));
			table.addElement(tr);
	...
	public String getInstructions(WebSession s) {
		...
		String instructions = "";
		if (chinese) {
			instructions = "在并发的环节里面，我们也可以利用低价买高价商品~来试试看 :)";
		} else {
			instructions = "For this exercise, your mission is to exploit the concurrency issue which will allow you to purchase merchandise for a lower price.";
		}
		return (instructions);
	}
