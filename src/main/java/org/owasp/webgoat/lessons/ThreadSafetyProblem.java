...
public class ThreadSafetyProblem extends LessonAdapter {
	...
	public String getInstructions(WebSession s) {
		...
		String instructions = "";
		if (chinese) {
			instructions = "用户通过登陆之后可以查看到对应的用户信息，测试用多个用户同时进行登陆操作，看看会发生什么呢？"
					+"用户名分别是'jeff‘和'dave'，<b>最好使用2个浏览器同时进行提交操作。</b>";
		} else {
			instructions = "The user should be able to exploit the concurrency error in this web application "
