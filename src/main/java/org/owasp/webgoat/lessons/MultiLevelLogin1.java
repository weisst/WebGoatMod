...
public class MultiLevelLogin1 extends SequentialLessonAdapter {
	...
	protected Element createMainContent(WebSession s) {
		...
		try {
			...
			sty.addElement("#lesson_password_tan input {width: 100px; margin-top: 20px; margin-left: 10px;}        "
					+ "#lesson_wrapper {height: 435px;width: "
					...
					+ "#lesson_submit input {margin-top: 30px;}         "
	...
	private Element createWorkspaceContent(WebSession s) {
		...
		// user is loggedIn but enters wrong tan
				...
				&& !correctTan(getLoggedInUser(s), tan, hiddenTan, s)) {
		...
		if (loggedIn(s) && correctTan(s)) {
			...
			if (!(user + password).equals("")) {
				errorMessage = "Login failed! Make sure "
	 ...
	 * @param tanNr
	 */
	private void createAskForTanContent(WebSession s, ElementContainer ec,
			int tanNr) {
		...
		TR tr1 = new TR();
		TD td1 = new TD();
		td1.addAttribute("style",
				"padding-top: 20px;text-align:left;padding-left: 50px;");
		TD td2 = new TD();
	...
	private void createNoTanLeftContent(ElementContainer ec) {
		...
		ec.addElement(new BR());
		H1 h = new H1(
				"<center>No tan is left! Please contact the admin. </center>");
		ec.addElement(h);
		ec.addElement(createLogoutLink());
	}
	 ...
	 */
	private boolean correctTan(String user, String tan, String tanPosition,
			WebSession s) {
		if (tan.equals("")) {
			return false;
		}
	...
	public String getInstructions(WebSession s) {
		...
		if (getLessonTracker(s).getStage() == 1) {
			if (chinese) {
				instructions = "STAGE 1:\t 在这节中将展示让\"失效\"数据再次使用的经典漏洞 ，"
						+ "首先我们使用用户名 <b>Jane</b> 和密码<b>tarzan</b>进行登陆查看我们的TANs记录，每条TAN记录只能查看<b>一次</b>！<br> "
						+ "下面是Jane的TANs: <br>" + "Tan #1 = 15648<br>"
						+ "Tan #2 = 92156<br>" + "Tan #3 = 4879<br>"
						+ "Tan #4 = 9458<br>" + "Tan #5 = 4879<br>";
			} else {
			...
			if (chinese) {
				instructions = "STAGE 2:\t现在如果我们再次查看刚刚已经查看过的TAN记录，系统会提示不成功，是否可以让\"失效\"的数据再次展现呢？";
			} else {
				instructions = "STAGE 2:\tNow you are a hacker who "
						+ "already has stolen some information from Jane by "
						+ "a phishing mail. "
						+ "You have the password which is tarzan and "
						+ "the Tan #1 which is 15648 <br>"
						+ "The problem is that the first tan is already "
